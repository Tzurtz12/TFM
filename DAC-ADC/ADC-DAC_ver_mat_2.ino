...
String operations[Noperations] = {"NOP", "INITIALIZE", "SET", "GET_DAC", "GET_ADC", "RAMP1", "RAMP2", "BUFFER_RAMP", "BUFFER_RAMP_DIS", "RESET", "TALK", "CONVERT_TIME", "*IDN?", "*RDY?", "GET_DUNIT","SET_DUNIT", "ADC_ZERO_SC_CAL", "ADC_CH_ZERO_SC_CAL", "ADC_CH_FULL_SC_CAL", "DAC_CH_CAL", "FULL_SCALE","TEST","OSCI"};
...
void test(){
  ...
  Serial.write(0xAA);
  Serial.write(SPI.transfer(adc,0));
  Serial.write(SPI.transfer(adc,0));
  Serial.write(SPI.transfer(adc,0));
  Serial.write(SPI.transfer(adc,0));
  Serial.write(SPI.transfer(adc,0));
...
void getSingleReadingOsci(){
  for(int i=0;i<4;i++){
    SPI.transfer(adc,0x38+i); //write to register what to do
    SPI.transfer(adc,0x4A); //configure single shot 24 bit
    waitDRDY(); //wait for conversion
    SPI.transfer(adc,0x48+i); //indicates you want to read
    Serial.write(SPI.transfer(adc,0)); //status
    Serial.write(SPI.transfer(adc,0)); //byte 1
    Serial.write(SPI.transfer(adc,0)); //byte 2
    Serial.write(SPI.transfer(adc,0)); //byte 3
  }
}

void longToByteSend(long s){
  Serial.write((byte)((s>>24)&0xFF));
  Serial.write((byte)((s>>16)&0xFF));
...
void osci(){
  ...
  int j=0;
  while(run_loop && j<1000){
    j=j+1;
    long time_v = millis();       //gets the timing of the point 
    Serial.write(0xAA); //start comm just to know where you are   
    longToByteSend(time_v);       //converts the long and sends it to the serial
    getSingleReadingOsci();
    Serial.write(0xBB); //end comm just to know where you are
    //used to stop the loop so arduino runs not infinitly
    ...
    if(m==(byte)0x41){
      ...
      if(m==(byte)0x16){
        ...
        run_loop=false;
        //nothing good came back break the loop 
      }
    }
  }
}
